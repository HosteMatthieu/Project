...
class BlockchainNode(Node):
    ...
    def new_certificate(self, certificate):
        if not certificate.is_legit():  # Vérifie si le certificat est légitime
            return

        if certificate in self.__certificateBox:  # Vérifie s'il est déjà dans la boîte
            return

        if self.blockchain.contains_certificate(certificate):  # Vérifie s'il est déjà dans la blockchain
            return

        self.__certificateBox.add(certificate)

        if not self.try_forge_next_block():  # Essaie de forger un nouveau bloc
            print(f"Échec du forger du bloc. Diffusion du certificat : {certificate}")
            self.broadcast_object(certificate)
        else:
            print(f"Bloc forgé avec succès. Certificat ajouté à la blockchain : {certificate}")
